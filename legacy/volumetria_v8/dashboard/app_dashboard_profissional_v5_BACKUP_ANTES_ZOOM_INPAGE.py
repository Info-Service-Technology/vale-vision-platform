...
def render_static_image_path(path_str: str, caption: str | None = None):
    ...
    st.markdown(
        f"<a href='{href}' target='_blank' title='Abrir em tela grande' style='display:block;'>"
        f"<img src='{href}' style='width:100%; border-radius:16px; border:1px solid rgba(255,255,255,0.08); display:block; cursor:zoom-in;' />"
        f"</a>",
        unsafe_allow_html=True,
    )
    st.caption(caption or 'Clique na imagem para abrir em tela grande.')
    st.markdown(
        f"<a href='{href}' target='_blank' style='display:inline-block; margin-top:4px; padding:8px 12px; border-radius:10px; background:#1d4ed8; color:#ffffff; text-decoration:none; font-weight:700; border:1px solid rgba(255,255,255,0.14);'>Abrir imagem grande</a>",
        unsafe_allow_html=True,
    )
