...
st.set_page_config(
    page_title="Vale | Dashboard Operacional V4",
    page_icon="📊",
    layout="wide",
...
def fill_operacional(status, fill, motivo):
    ...
    # fallback operacional forte para o caso que está te incomodando
...
if "selected_event_id_v4" not in st.session_state:
    st.session_state.selected_event_id_v4 = None

with st.sidebar:
    st.header("Painel")
    auto_refresh = st.checkbox("Atualização automática", value=True)
    intervalo = st.selectbox("Intervalo", [15, 30, 60, 120], index=0)
...
if auto_refresh:
    st_autorefresh(interval=intervalo * 1000, key="auto_refresh_dashboard_v4")

top1, top2 = st.columns([0.82, 0.18])
with top1:
    st.title("Projeto Vale — Dashboard Operacional")
    st.caption("Release V8 | camada operacional V4")
with top2:
    st.markdown(
        f"""
...
for start in range(0, len(ult20), 4):
    ...
    for idx, (_, row) in enumerate(bloco.iterrows()):
        with cols[idx]:
            ...
            if st.button("Ver detalhe", key=f"v4_det_{int(row['id'])}", use_container_width=True):
                st.session_state.selected_event_id_v4 = int(row["id"])
...
if st.session_state.selected_event_id_v4 is None:
    st.session_state.selected_event_id_v4 = int(ultimo["id"])

row_sel_df = f[f["id"] == st.session_state.selected_event_id_v4]
...
with d1:
    ...
    meta_cols = [
        "arquivo_nome", "grupo", "fill_tecnico_exibicao", "fill_operacional_exibicao",
        "estado_dashboard", "acao_operacional", "motivo_falha",
        "materiais_detectados_raw", "contaminantes_detectados", "ts_processamento"
    ]
    for c in meta_cols:
