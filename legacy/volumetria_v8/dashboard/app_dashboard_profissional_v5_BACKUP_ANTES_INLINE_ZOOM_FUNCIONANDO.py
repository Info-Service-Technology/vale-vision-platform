...
def _labels_from_text(value: str):
    raw = str(value or '').strip()
    if not raw:
        return set()
    parts = re.split(r'[,;|]+', raw)
    return {_normalize_label(x) for x in parts if str(x).strip()}


def _set_zoom_image_href(href: str, title: str = "Imagem ampliada"):
    st.session_state["zoom_image_href"] = href
    st.session_state["zoom_image_title"] = title


def _render_zoom_button(href: str, key: str | None, title: str):
    if key and st.button("Abrir imagem grande", key=f"btn_zoom_{key}", use_container_width=True):
        _set_zoom_image_href(href, title)
        st.rerun()


def render_static_image_path(path_str: str, caption: str | None = None, key: str | None = None, title: str = "Imagem ampliada"):
    p = Path(str(path_str))
    if not p.exists():
    ...
    suffix = p.suffix.lower().lstrip('.') or 'jpg'
    mime = 'jpeg' if suffix in ('jpg', 'jpeg') else suffix
    href = f"data:image/{mime};base64,{b64}"
    st.markdown(
        f"<img src='{href}' style='width:100%; border-radius:16px; border:1px solid rgba(255,255,255,0.08); display:block;' />",
        unsafe_allow_html=True,
    )
    if caption:
        st.caption(caption)
    _render_zoom_button(href, key, title)


def render_static_image_array(img_rgb, caption: str | None = None, key: str | None = None, title: str = "Imagem ampliada"):
    if img_rgb is None:
        st.info('Sem imagem disponível.')
        return
    ok, buf = cv2.imencode('.jpg', cv2.cvtColor(img_rgb, cv2.COLOR_RGB2BGR), [int(cv2.IMWRITE_JPEG_QUALITY), 92])
    if not ok:
        st.info('Falha ao renderizar imagem.')
        return
...
def build_contaminant_overlay(debug_path_str: str, payload: str, grupo: str = '', contaminantes_detectados: str = '', tipo_contaminacao: str = ''):
    ...
    grupo_norm = _normalize_label(grupo)
    contam_labels = _labels_from_text(contaminantes_detectados)
    tipo_norm = _normalize_label(tipo_contaminacao)
    if tipo_norm:
        contam_labels.add(tipo_norm)
    ...
    for det in deteccoes:
        ...
        # desenha somente contaminante, nunca o material principal dominante
        if contam_labels:
            if label_norm not in contam_labels:
                continue
        else:
            if grupo_norm and label_norm == grupo_norm:
                continue

        conf = _maybe_number(det.get("conf") or det.get("confianca"))
        ...
        found.append({"label": label_norm, "area_ratio": area_ratio, "conf": conf, "kind": geom_type})
        count_drawn += 1
    ...
    overlay = cv2.addWeighted(shadow, 0.28, overlay, 0.72, 0)
...
with c1:
    ...
    if debug_path and Path(str(debug_path)).exists():
        render_static_image_path(
            str(debug_path),
            key=f"ultimo_debug_{ultimo.get('arquivo_nome','ultimo')}",
            title="Última análise ampliada",
        )
    else:
...
for start in range(0, len(ult20), 4):
    ...
    for idx, (_, row) in enumerate(bloco.iterrows()):
        with cols[idx]:
            ...
            if debug_path and Path(str(debug_path)).exists():
                render_static_image_path(str(debug_path))
            else:
                st.info("Sem debug")
...
with d2:
    ...
    if debug_path and Path(str(debug_path)).exists():
        ...
        render_static_image_path(
            str(debug_path),
            key=f"detail_debug_{row_sel.get('arquivo_nome','')}",
            title="Imagem debug ampliada",
        )
    else:
        st.info("Sem imagem debug disponível.")
...
with d3:
    ...
    overlay_img, found = build_contaminant_overlay(
        ...
        row_sel.get("grupo", ""),
        row_sel.get("contaminantes_detectados", ""),
        row_sel.get("tipo_contaminacao", ""),
    )
    if overlay_img is not None:
        render_static_image_array(
            overlay_img,
            key=f"detail_cont_{row_sel.get('arquivo_nome','')}",
            title="Contaminante demarcado ampliado",
        )
        txt = format_found_contaminants(found)
        if txt:
            st.caption(txt)
    else:
        st.info("Sem geometria de contaminante válida dentro da caçamba para demarcação.")

if st.session_state.get("zoom_image_href"):
    st.markdown("---")
    z1, z2 = st.columns([10, 2])
    with z1:
        st.markdown(f"### {st.session_state.get('zoom_image_title', 'Imagem ampliada')}")
    with z2:
        if st.button("Fechar imagem grande", key="btn_close_zoom_image", use_container_width=True):
            st.session_state.pop("zoom_image_href", None)
            st.session_state.pop("zoom_image_title", None)
            st.rerun()

    st.markdown(
        f"<img src='{st.session_state['zoom_image_href']}' style='width:100%; border-radius:18px; border:1px solid rgba(255,255,255,0.10); display:block;' />",
        unsafe_allow_html=True,
    )
