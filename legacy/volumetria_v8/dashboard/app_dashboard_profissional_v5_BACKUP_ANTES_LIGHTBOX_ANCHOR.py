...
def _toggle_inline_zoom(key: str, visible: bool | None = None):
    skey = f"inline_zoom_visible_{key}"
    if visible is None:
        st.session_state[skey] = not bool(st.session_state.get(skey, False))
    else:
        st.session_state[skey] = bool(visible)


def _render_inline_zoom_controls_path(path_str: str, key: str | None, title: str):
    if not key:
        return

    visible_key = f"inline_zoom_visible_{key}"
    is_visible = bool(st.session_state.get(visible_key, False))
    label = "Voltar ao tamanho normal" if is_visible else "Expandir imagem"

    if st.button(label, key=f"btn_zoom_toggle_{key}", use_container_width=True):
        _toggle_inline_zoom(key)
        is_visible = bool(st.session_state.get(visible_key, False))

    if is_visible:
        st.caption(title)
        st.image(str(path_str), use_container_width=True)


def _render_inline_zoom_controls_array(img_rgb, key: str | None, title: str):
    if not key or img_rgb is None:
        return
...
def render_static_image_path(path_str: str, caption: str | None = None, key: str | None = None, title: str = "Imagem ampliada"):
    ...
    if caption:
        st.caption(caption)
    _render_inline_zoom_controls_path(str(p), key, title)
...
def render_static_image_array(img_rgb, caption: str | None = None, key: str | None = None, title: str = "Imagem ampliada"):
    ...
    if caption:
        st.caption(caption)
    _render_inline_zoom_controls_array(img_rgb, key, title)
...
with c1:
    ...
    if debug_path and Path(str(debug_path)).exists():
        render_static_image_path(
            ...
            title="Última análise expandida",
        )
    else:
...
with d3:
    ...
    if overlay_img is not None:
        render_static_image_array(
            ...
            title="Contaminante demarcado expandido",
        )
