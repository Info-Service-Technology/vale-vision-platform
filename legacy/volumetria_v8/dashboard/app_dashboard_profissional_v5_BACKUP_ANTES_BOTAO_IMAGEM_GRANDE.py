...
def render_static_image_path(path_str: str, caption: str | None = None):
    p = Path(str(path_str))
    if not p.exists():
    ...
    st.markdown(
        f"<a href='data:image/{mime};base64,{b64}' target='_blank' title='Abrir em tela grande' style='display:block;'>"
        f"<img src='data:image/{mime};base64,{b64}' style='width:100%; border-radius:16px; border:1px solid rgba(255,255,255,0.08); display:block; cursor:zoom-in;' />"
        f"</a>",
        unsafe_allow_html=True,
    )
    st.caption(caption or 'Clique na imagem para abrir em tela grande.')


def render_static_image_array(img_rgb, caption: str | None = None):
    if img_rgb is None:
        st.info('Sem imagem disponível.')
        return
    ...
    st.markdown(
        f"<a href='data:image/jpeg;base64,{b64}' target='_blank' title='Abrir em tela grande' style='display:block;'>"
        f"<img src='data:image/jpeg;base64,{b64}' style='width:100%; border-radius:16px; border:1px solid rgba(255,255,255,0.08); display:block; cursor:zoom-in;' />"
        f"</a>",
        unsafe_allow_html=True,
    )
    st.caption(caption or 'Clique na imagem para abrir em tela grande.')
...
with d3:
    ...
    if overlay_img is not None:
        render_static_image_array(overlay_img)
