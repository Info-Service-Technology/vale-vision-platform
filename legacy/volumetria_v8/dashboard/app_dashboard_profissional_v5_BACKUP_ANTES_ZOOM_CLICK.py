...
def render_static_image_path(path_str: str, caption: str | None = None):
    ...
    st.markdown(
        f"<img src='data:image/{mime};base64,{b64}' style='width:100%; border-radius:16px; border:1px solid rgba(255,255,255,0.08); display:block;' />",
        unsafe_allow_html=True,
    )
    if caption:
        st.caption(caption)
...
def render_static_image_array(img_rgb, caption: str | None = None):
    ...
    st.markdown(
        f"<img src='data:image/jpeg;base64,{b64}' style='width:100%; border-radius:16px; border:1px solid rgba(255,255,255,0.08); display:block;' />",
        unsafe_allow_html=True,
    )
    if caption:
        st.caption(caption)
