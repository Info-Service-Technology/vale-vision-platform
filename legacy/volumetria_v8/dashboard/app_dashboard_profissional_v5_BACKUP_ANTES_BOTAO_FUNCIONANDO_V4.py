...
def _render_inline_zoom_controls(href: str, key: str | None, title: str):
    if not key:
        return
    ...
    if not is_visible:
        if st.button("Abrir imagem grande", key=f"btn_zoom_open_{key}", use_container_width=True):
            _toggle_inline_zoom(key, True)
            st.rerun()
        return
    ...
    with c1:
        st.caption(title)
    with c2:
        if st.button("Fechar imagem grande", key=f"btn_zoom_close_{key}", use_container_width=True):
            _toggle_inline_zoom(key, False)
            st.rerun()

    st.markdown(
        f"<img src='{href}' style='width:100%; border-radius:18px; border:1px solid rgba(255,255,255,0.10); display:block; margin-top:8px;' />",
        unsafe_allow_html=True,
    )
...
def render_static_image_path(path_str: str, caption: str | None = None, key: str | None = None, title: str = "Imagem ampliada"):
    ...
    if caption:
        st.caption(caption)
    _render_inline_zoom_controls(href, key, title)
