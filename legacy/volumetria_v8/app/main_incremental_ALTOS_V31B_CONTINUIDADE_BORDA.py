...
class HighGeometryPointRefiner:
    ...
    def _should_preserve_original_on_rim_break(self, gate_info, orb_info, fill_old):
        """
        Quando a carga corta a continuidade visual da borda, o refinador por pontos
        não deve degradar um frame que já estava OK no pipeline base.
        Nestes casos, preservamos o resultado original em vez de marcar suspeito.
        """
        detalhe = str((gate_info or {}).get("detalhe", "") or "")
        motivo = str((gate_info or {}).get("motivo", "") or "")
        matches = int((orb_info or {}).get("matches", 0) or 0)
        ...
        # Caso clássico: material corta a borda e faz a linha encostar no topo/lateral.
        ...
        # Caso clássico: a borda superior some atrás da carga e a boca colapsa.
        if detalhe in (
            "estrutura_pequena",
        ...
        # Rejeições de referencia desalinhada, mas com ORB ainda razoável, também podem ser só borda interrompida.
    ...
    def maybe_refine(self, img_path, img_bgr, grupo, opening_repair, floor, wall, feats_atual):
        ...
        if not gate_ok:
            ...
            elif self._should_preserve_original_on_rim_break(gate_info, info, fill_old):
                # Mantém o resultado do pipeline base para não quebrar a apresentação
                # quando a carga interrompe a continuidade visual da borda.
