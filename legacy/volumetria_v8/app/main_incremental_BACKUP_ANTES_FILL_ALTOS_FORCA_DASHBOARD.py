...
def calibrar_fill_operacional(grupo, fill_tecnico):
    ...
    # Calibracao operacional focada em caçambas altas / casos comerciais de topo.
    # Nao altera o fill tecnico; so o valor exibido e usado para logica operacional.
    if grupo == "plastico":
        if f >= 92.0:
            return min(100.0, f)
        if f >= 85.0:
        ...
        if f >= 80.0:
            return min(100.0, f + 6.5)
        if f >= 74.0:
            return min(100.0, f + 6.0)
...
def main():
    ...
    for img_path, img, opening, floor, wall in stage:
        ...
        feats["fill_percent_filtered"] = fill_tecnico
        ...
        row = base.normalize_row({
            ...
            "fill_percent_filtrado": f"{fill_tecnico:.6f}",
