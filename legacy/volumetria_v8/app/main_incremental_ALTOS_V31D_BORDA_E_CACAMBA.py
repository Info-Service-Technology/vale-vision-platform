...
class HighGeometryPointRefiner:
    ...
    PRESERVE_BASE_MIN_OPENING_W_RATIO_VS_ENV = 0.55
    PRESERVE_BASE_MIN_OPENING_H_RATIO_VS_ENV = 0.12
    PRESERVE_BASE_MIN_OPENING_AREA_RATIO_VS_ENV = 0.10
    PRESERVE_BASE_MIN_INSIDE_RATIO = 0.82
    PRESERVE_BASE_MAX_CENTER_SHIFT_X = 0.22
    PRESERVE_BASE_MAX_CENTER_SHIFT_Y = 0.22

    EMPTY_FLOOR_MAX_FILL = 18.0
    EMPTY_FLOOR_MIN_FLOOR_VS_OPENING = 0.68
    EMPTY_FLOOR_MAX_WALL_VS_OPENING = 0.08
    EMPTY_FLOOR_MIN_OPENING_W_RATIO = 0.22
    EMPTY_FLOOR_MIN_OPENING_H_RATIO = 0.38
    EMPTY_FLOOR_HARD_MAX_FILL = 22.0
    EMPTY_FLOOR_HARD_MIN_FLOOR_VS_OPENING = 0.55
    EMPTY_FLOOR_HARD_MAX_WALL_VS_OPENING = 0.18
    EMPTY_FLOOR_HARD_MIN_OPENING_W_RATIO = 0.38
    EMPTY_FLOOR_HARD_MIN_OPENING_H_RATIO = 0.45
    EMPTY_FLOOR_HARD_MIN_OPENING_AREA_RATIO = 0.12
    ...
    def _assess_base_opening_quality(self, opening_repair, floor, wall, img_shape):
        opening = self._largest_component(self._mask255(opening_repair))
        if opening is None:
            return False, {"detalhe": "base_opening_vazia"}

        support = self._build_structural_support(floor, wall)
        if support is None:
            return False, {"detalhe": "base_sem_estrutura"}

        obb = self._bbox(opening)
        sbb = self._bbox(support)
        if obb is None or sbb is None:
            return False, {"detalhe": "base_bbox_invalida"}

        x1, y1, x2, y2, ow, oh = obb
        sx1, sy1, sx2, sy2, sw, sh = sbb
        oarea = int(np.count_nonzero(opening > 0))
        sarea = int(np.count_nonzero(support > 0))
        opening_w_ratio_vs_env = ow / float(max(1, sw))
        opening_h_ratio_vs_env = oh / float(max(1, sh))
        opening_area_ratio_vs_env = oarea / float(max(1, sarea))
        ...
        k = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (dilate_px, dilate_px))
        support_guard = cv2.dilate(support, k, iterations=1)
        inside_ratio = self._inside_ratio(opening, support_guard)

        h, w = img_shape[:2]
        base_cx = ((x1 + x2) / 2.0) / float(max(1, w))
        base_cy = ((y1 + y2) / 2.0) / float(max(1, h))
        env_cx = ((sx1 + sx2) / 2.0) / float(max(1, w))
        env_cy = ((sy1 + sy2) / 2.0) / float(max(1, h))
        center_shift_x = abs(base_cx - env_cx)
        center_shift_y = abs(base_cy - env_cy)

        _, border_flags, _ = self._touches_border(opening)
        lateral_touch = bool(border_flags.get("left")) or bool(border_flags.get("right"))
        top_touch = bool(border_flags.get("top"))

        ok = (
            opening_w_ratio_vs_env >= self.PRESERVE_BASE_MIN_OPENING_W_RATIO_VS_ENV
            and opening_h_ratio_vs_env >= self.PRESERVE_BASE_MIN_OPENING_H_RATIO_VS_ENV
            and opening_area_ratio_vs_env >= self.PRESERVE_BASE_MIN_OPENING_AREA_RATIO_VS_ENV
            and inside_ratio >= self.PRESERVE_BASE_MIN_INSIDE_RATIO
            and center_shift_x <= self.PRESERVE_BASE_MAX_CENTER_SHIFT_X
            and center_shift_y <= self.PRESERVE_BASE_MAX_CENTER_SHIFT_Y
            and not (lateral_touch and top_touch)
        )
        return ok, {
            "detalhe": "base_opening_ok" if ok else "base_opening_fraca",
            "opening_w_ratio_vs_env": float(opening_w_ratio_vs_env),
            "opening_h_ratio_vs_env": float(opening_h_ratio_vs_env),
            "opening_area_ratio_vs_env": float(opening_area_ratio_vs_env),
            "inside_ratio": float(inside_ratio),
            "center_shift_x": float(center_shift_x),
            "center_shift_y": float(center_shift_y),
            "border_flags": border_flags,
        }

    def _detect_empty_floor_or_missing_dumpster(self, opening_repair, floor, wall, feats_atual, img_shape):
        try:
        ...
        if obb is None:
            return False, {}
        x1, y1, x2, y2, ow, oh = obb
        ow_ratio = ow / float(max(1, w))
        oh_ratio = oh / float(max(1, h))
        ...
        floor_vs_opening = floor_area / float(max(1, opening_area))
        wall_vs_opening = wall_area / float(max(1, opening_area))

        _, border_flags, _ = self._touches_border(opening)
        top_touch = bool(border_flags.get("top"))
        bottom_touch = bool(border_flags.get("bottom"))
        lateral_touch = bool(border_flags.get("left")) or bool(border_flags.get("right"))

        hard_absent = (
            fill_old <= self.EMPTY_FLOOR_HARD_MAX_FILL
            and floor_vs_opening >= self.EMPTY_FLOOR_HARD_MIN_FLOOR_VS_OPENING
            and wall_vs_opening <= self.EMPTY_FLOOR_HARD_MAX_WALL_VS_OPENING
            and ow_ratio >= self.EMPTY_FLOOR_HARD_MIN_OPENING_W_RATIO
            and oh_ratio >= self.EMPTY_FLOOR_HARD_MIN_OPENING_H_RATIO
            and (opening_area / float(max(1, h * w))) >= self.EMPTY_FLOOR_HARD_MIN_OPENING_AREA_RATIO
        )

        is_empty_like = hard_absent or (
            fill_old <= self.EMPTY_FLOOR_MAX_FILL
            and floor_vs_opening >= self.EMPTY_FLOOR_MIN_FLOOR_VS_OPENING
            and wall_vs_opening <= self.EMPTY_FLOOR_MAX_WALL_VS_OPENING
            and ow_ratio >= self.EMPTY_FLOOR_MIN_OPENING_W_RATIO
            and oh_ratio >= self.EMPTY_FLOOR_MIN_OPENING_H_RATIO
            and (top_touch or lateral_touch or (top_touch and bottom_touch))
        )
        return is_empty_like, {
            "fill_old": float(fill_old),
            "floor_vs_opening": float(floor_vs_opening),
            "wall_vs_opening": float(wall_vs_opening),
            "opening_w_ratio": float(ow_ratio),
            "opening_h_ratio": float(oh_ratio),
            "opening_area_ratio_img": float(opening_area / float(max(1, h * w))),
            "border_flags": border_flags,
            "hard_absent": bool(hard_absent),
        }
    ...
    def _should_preserve_original_on_rim_break(self, gate_info, orb_info, fill_old, opening_repair, floor, wall, img_shape):
        """
        Preserve o resultado-base quando a borda foi interrompida pela carga,
        desde que o original ainda esteja estruturalmente plausível.
        """
        detalhe = str((gate_info or {}).get("detalhe", "") or "")
        motivo = str((gate_info or {}).get("motivo", "") or "")
        matches = int((orb_info or {}).get("matches", 0) or 0)
        ...
        base_ok, base_info = self._assess_base_opening_quality(opening_repair, floor, wall, img_shape)
        if fill_old < 28.0:
            return False

        if detalhe.startswith("boca_encosta_borda_"):
            has_top = "top': True" in detalhe
            has_left = "left': True" in detalhe
            has_right = "right': True" in detalhe
            both_sides = has_left and has_right
            if has_top and (has_left or has_right) and not both_sides:
                if matches >= 70 and inliers >= 35 and inlier_ratio >= 0.42:
                    if base_ok:
                        return True
                    # fallback parcial para borda interrompida clássica
                    if base_info.get("inside_ratio", 0.0) >= 0.70 and base_info.get("opening_w_ratio_vs_env", 0.0) >= 0.32:
                        return True
                return False
            if both_sides:
                return False
        ...
        if detalhe in (
            "estrutura_pequena",
            "geometria_boca_pequena_relativa",
            "geometria_boca_muito_pequena_abs",
            "boca_muito_pequena",
        ):
            if matches >= 42 and inliers >= 12:
                return True
            if fill_old >= 40.0 and matches >= 34 and inliers >= 10:
                return True

        if motivo == "referencia_desalinhada" and matches >= 45 and inliers >= 14 and fill_old >= 35.0:
            return True
    ...
    def maybe_refine(self, img_path, img_bgr, grupo, opening_repair, floor, wall, feats_atual):
        ...
        empty_like, empty_info = self._detect_empty_floor_or_missing_dumpster(opening_repair, floor, wall, feats_atual, img_bgr.shape)
        if empty_like:
            feats_vazio = self._mark_feats_as_suspect(feats_atual, "suspeito_cacamba_ausente")
            return feats_vazio, opening_repair, {**empty_info, "motivo": "suspeito_cacamba_ausente"}
        ...
        if not gate_ok:
            ...
            elif self._should_preserve_original_on_rim_break(gate_info, info, fill_old, opening_repair, floor, wall, img_bgr.shape):
                # Mantém o resultado do pipeline base apenas quando o original já estiver
                # estruturalmente aceitável e a borda tiver sido interrompida pela carga.
                return feats_atual, opening_repair, {**info, **gate_info, "fill_old": fill_old, "motivo": "rim_continuity_preserve_original"}
            else:
        ...
        if motivo_antigo:
            ...
            feats_v3["motivo_falha"] = "altos_v31d_borda_e_cacamba"
...
def main():
    ...
    if not new_image_files:
        ...
        run_info = {
            ...
            "mode": "incremental_altos_v31d_borda_e_cacamba",
    ...
    print(f"ALTOS_V31C_CONTROLE_FINAL = {'ATIVO' if high_geom_v3.ativo else 'INATIVO'}")
