...
def humano_intersecta_abertura(
    ...
    for det in detections:
        ...
        # precisa cruzar a borda da abertura:
    ...
    for img_path, img, opening, floor, wall in stage:
        ...
        if human_hit:
            ...
            row = normalize_row({
                ...
                "suspeita_floor_filtrado_colapsou": "",
                "materiais_detectados_raw": "",
                "deteccoes_contaminantes_json": json.dumps([], ensure_ascii=False),
                "contaminantes_detectados": "",
                "alerta_contaminacao": 0,
                "tipo_contaminacao": "",
                "severidade_contaminacao": 0,
                "cacamba_esperada": grupo,
                "material_esperado": grupo,
            })
            new_rows.append(row)

            print(
                f"{img_path.name} -> grupo={grupo} | status=suspeito | "
                f"fill= | estado=revisar | ok_consec=0 | alerta=0 | motivo=humano_na_abertura"
            )
            continue
