...
class HighGeometryPointRefiner:
    ...
    GATE_MIN_CAND_W_RATIO = 0.24
    GATE_MIN_CAND_H_RATIO = 0.06
    GATE_MAX_CAND_W_RATIO = 0.95
    GATE_MAX_CAND_H_RATIO = 0.55

    GATE_MIN_STRUCT_AREA_RATIO = 0.018
    GATE_MIN_STRUCT_W_RATIO = 0.30
    GATE_MIN_STRUCT_H_RATIO = 0.10

    GATE_MIN_INSIDE_RATIO = 0.74
    ...
    ACCEPT_MIN_DELTA_FILL = 8.0
    ACCEPT_MIN_FILL_V3 = 68.0
    ...
    def _run_commercial_gate(self, warped_mask, floor, wall, img_shape):
        ...
        if bw_ratio < self.GATE_MIN_CAND_W_RATIO or bh_ratio < self.GATE_MIN_CAND_H_RATIO:
            return False, None, {"motivo": "referencia_desalinhada", "detalhe": "geometria_boca_pequena", "inside_ratio": 0.0}
        cand_border_touch, cand_border_flags, _ = self._touches_border(cand)
        cand_bottom_touch_only = (
            cand_border_touch and bool(cand_border_flags.get("bottom"))
        ...
        if cand_border_touch and not (self.GATE_ALLOW_BOTTOM_TOUCH_ONLY and cand_bottom_touch_only):
            return False, None, {"motivo": "frame_parcial", "detalhe": f"boca_encosta_borda_{cand_border_flags}", "inside_ratio": 0.0}
        support = self._build_structural_support(floor, wall)
        if support is None:
        ...
        if inside_ratio < self.GATE_MIN_INSIDE_RATIO:
            ...
            return False, support, {"motivo": motivo, "detalhe": f"boca_fora_envelope_{inside_ratio:.3f}", "inside_ratio": float(inside_ratio)}
    ...
    def maybe_refine(self, img_path, img_bgr, grupo, opening_repair, floor, wall, feats_atual):
        ...
        candidato = (
            fill_old >= 30.0
            and fill_old <= 78.0
            and floor_vs_opening <= 0.16
            and wall_vs_opening >= 0.05
        )
        if not candidato:
            return feats_atual, opening_repair, None
        ...
        if motivo_antigo:
            ...
            feats_v3["motivo_falha"] = "altos_v3_pontos"
        ...
        txt = (
            f"OLD={fill_old:.2f} | V3={fill_v3:.2f} | m={info.get('matches',0)} | "
...
def main():
    ...
    if not new_image_files:
        ...
        run_info = {
            ...
            "mode": "incremental_altos_v3_pontos",
    ...
    print(f"ALTOS_V3_PONTOS = {'ATIVO' if high_geom_v3.ativo else 'INATIVO'}")
