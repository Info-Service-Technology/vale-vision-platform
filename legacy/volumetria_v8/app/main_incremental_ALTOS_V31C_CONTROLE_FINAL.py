...
class HighGeometryPointRefiner:
    ...
    def _should_preserve_original_on_rim_break(self, gate_info, orb_info, fill_old, opening_repair, floor, wall, img_shape):
        """
        Preserve o resultado-base apenas quando o original já estiver geometricamente aceitável.
        Isso evita salvar casos magros/lateralizados e reduz falsos positivos em altos ruins.
        """
        detalhe = str((gate_info or {}).get("detalhe", "") or "")
        motivo = str((gate_info or {}).get("motivo", "") or "")
        matches = int((orb_info or {}).get("matches", 0) or 0)
        ...
        base_ok, _base_info = self._assess_base_opening_quality(opening_repair, floor, wall, img_shape)
        if not base_ok:
            return False
        ...
        if detalhe in (
            ...
            if matches >= 40 and inliers >= 12:
                return True
            if fill_old >= 40.0 and matches >= 32 and inliers >= 10:
                return True
    ...
    def maybe_refine(self, img_path, img_bgr, grupo, opening_repair, floor, wall, feats_atual):
        ...
        if motivo_antigo:
            ...
            feats_v3["motivo_falha"] = "altos_v31c_controle_final"
...
def main():
    ...
    if not new_image_files:
        ...
        run_info = {
            ...
            "mode": "incremental_altos_v31c_controle_final",
