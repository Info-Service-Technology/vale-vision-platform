...
AUTO_REVIEW_CONT_MADEIRA_SUCATA = AUTO_REVIEW_ROOT / "madeira_contaminante_sucata"
AUTO_REVIEW_BOCA_MADEIRA = AUTO_REVIEW_ROOT / "madeira_sem_boca_v2"
...
def _ensure_auto_review_dirs():
    ...
    AUTO_REVIEW_CONT_MADEIRA_SUCATA.mkdir(parents=True, exist_ok=True)
    AUTO_REVIEW_BOCA_MADEIRA.mkdir(parents=True, exist_ok=True)
...
def _classify_auto_review_case(row: dict) -> str:
    ...
    alerta_cont = str(row.get("alerta_contaminacao", "")).strip()

    if grupo == "madeira" and "sucata" in contaminantes and alerta_cont == "1":
        return "madeira_contaminante_sucata"

    if grupo == "madeira" and motivo == "sem_boca_modelo_v2":
        return "madeira_sem_boca_v2"

    return ""


def _append_auto_review_manifest(case_type: str, img_path: Path, row: dict):
...
def auto_collect_review_case(img_path: Path, row: dict):
    case_type = _classify_auto_review_case(row)
    if not case_type:
        return

    _ensure_auto_review_dirs()

    if case_type == "madeira_contaminante_sucata":
        base_dir = AUTO_REVIEW_CONT_MADEIRA_SUCATA
    elif case_type == "madeira_sem_boca_v2":
        base_dir = AUTO_REVIEW_BOCA_MADEIRA
    else:
        return
    ...
    _safe_copy(img_path, case_dir / img_path.name)

    debug_img = DEBUG_DIR / f"{img_path.stem}_debug.jpg"
    _safe_copy(debug_img, case_dir / f"{img_path.stem}_debug.jpg")
    ...
    _safe_copy(opening_mask, case_dir / f"{img_path.stem}_opening.png")
    _safe_copy(floor_mask, case_dir / f"{img_path.stem}_floor.png")
    _safe_copy(wall_mask, case_dir / f"{img_path.stem}_wall.png")
    ...
    _append_auto_review_manifest(case_type, img_path, row)
