...
def reparar_opening_fragmentada(opening_uint8, wall_uint8):
    ...
    if op_stats["area"] <= 0 or op_stats["bbox"] is None:
        ...
        if width_rel_vs_wall < 0.90:
            fragmentada = True
            info["motivos"].append("opening_estreita")

        if area_rel_vs_wall < 0.55:
            fragmentada = True
    ...
    if strong_trigger:
        aceita = (
            best_width_rel >= max(width_rel_vs_wall + 0.00, 0.68)
            and best_center_err <= 0.36
            and best_area_rel <= 1.30
        )
    else:
        aceita = (
            best_width_rel >= max(width_rel_vs_wall + 0.00, 0.74)
            and best_center_err <= max(center_err_before + 0.04, 0.26)
            and best_area_rel <= 1.22
        )
...
def main():
    ...
    for img_path, img, opening, floor, wall in stage:
        ...
        opening_bin_tmp = (opening_repair > 0).astype(np.uint8)
        floor_in_open_tmp = ((floor > 0) & (opening_bin_tmp > 0)).astype(np.uint8)
        wall_in_open_tmp = ((wall > 0) & (opening_bin_tmp > 0)).astype(np.uint8)

        opening_area_tmp = int(np.count_nonzero(opening_bin_tmp))
        floor_area_tmp = int(np.count_nonzero(floor_in_open_tmp))
        wall_area_tmp = int(np.count_nonzero(wall_in_open_tmp))
        ...
        aciona_resgate_pos_motor = (
            ...
            and estrutura_tmp > 0
            and floor_vs_opening_tmp <= 0.03
            and floor_share_tmp <= 0.12
            and wall_share_tmp >= 0.70
            and wall_share_tmp <= 0.98
            and wall_vs_opening_tmp >= 0.18
            and wall_vs_opening_tmp <= 0.72
            and expected_overlap_tmp >= 0.05
            and filtered_vs_raw_tmp >= 0.05
        )
        ...
        if aciona_resgate_pos_motor:
            ...
            row = _build_overflow_row(
                ...
                floor=(floor_in_open_tmp * 255).astype("uint8"),
                wall=(wall_in_open_tmp * 255).astype("uint8"),
