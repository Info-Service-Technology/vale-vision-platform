...
# =========================
# CALIBRACAO OPERACIONAL - CACAMBAS ALTAS
# Mantem fill tecnico intacto e ajusta apenas a exibicao/alerta operacional.
# =========================
CALIBRACAO_OPERACIONAL_ALTOS_ENABLED = True
...
def calibrar_fill_operacional(grupo, fill_tecnico):
    ...
    if not CALIBRACAO_OPERACIONAL_ALTOS_ENABLED:
        return max(0.0, min(100.0, f))

    # Ajuste suave nos enchimentos altos:
    # 74-81.999 -> +4 pp
    # 82+       -> +7 pp
    if f >= 82.0:
        f_op = f + 7.0
...
def main():
    ...
    if not new_image_files:
        ...
        "calibracao_operacional_altos_ativa": CALIBRACAO_OPERACIONAL_ALTOS_ENABLED,
            "calibracao_operacional_altos_ativa": CALIBRACAO_OPERACIONAL_ALTOS_ENABLED,
    ...
    for img_path, img, opening, floor, wall in stage:
        ...
        fill_operacional = calibrar_fill_operacional(grupo, fill)

        if feats["status_frame"] == "ok":
            estado_dashboard = estado_dashboard_from_fill(fill)
            estado_dashboard_operacional = estado_dashboard_from_fill(fill_operacional)
        elif feats["status_frame"] == "suspeito":
            estado_dashboard = "revisar"
            estado_dashboard_operacional = "revisar"
        else:
            estado_dashboard = "invalido"
            estado_dashboard_operacional = "invalido"
        ...
        feats["estado_dashboard_temporal"] = estado_dashboard_operacional
