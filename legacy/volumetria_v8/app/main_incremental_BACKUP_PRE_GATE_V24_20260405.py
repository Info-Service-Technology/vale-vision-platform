...
class PlasticHighORBRefiner:
    # V2.2:
    # - mantém bloqueio forte para lateral / topo / meia caçamba / envelope ruim
    # - afrouxa apenas a borda inferior da boca candidata quando a estrutura confirma caçamba válida
    ...
    SOFT_RESCUE_MIN_MATCHES = 85
    SOFT_RESCUE_MIN_INLIERS = 35
    SOFT_RESCUE_MIN_INLIER_RATIO = 0.35
    SOFT_RESCUE_MIN_INSIDE_RATIO = 0.74
    SOFT_RESCUE_STRONG_MIN_MATCHES = 120
    SOFT_RESCUE_STRONG_MIN_INLIERS = 50
    ...
    def _allow_soft_gate_rescue(self, gate_info, orb_info, fill_old):
        ...
        if detalhe.startswith("estrutura_clip_borda_"):
            if not self._detail_has_only_bottom_touch(detalhe):
                return False
            return matches >= self.SOFT_RESCUE_STRONG_MIN_MATCHES and inliers >= self.SOFT_RESCUE_STRONG_MIN_INLIERS

        # 2) borda inferior sem suporte suficiente, mas ORB muito forte
        if detalhe.startswith("borda_inferior_sem_suporte_"):
            return inside_ratio >= self.SOFT_RESCUE_MIN_INSIDE_RATIO

        # 3) estrutura pequena para alto plástico, desde que ORB esteja forte
...
def main():
    ...
    print()
    print("CONCLUIDO")
    print("MODO: incremental_altos_v2")
    print("CSV:", csv_path)
