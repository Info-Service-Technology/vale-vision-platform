...
# =========================
# COLETA AUTOMATICA DE CASOS PARA REVISAO
# - coleta candidatos a erro para qualquer grupo
# - foco atual: contaminacao e boca/opening
# =========================
AUTO_REVIEW_ROOT = DEBUG_DIR.parent / "revisao_automatica"
AUTO_REVIEW_MANIFEST = AUTO_REVIEW_ROOT / "manifest_revisao.csv"
AUTO_REVIEW_CONT_DIR = AUTO_REVIEW_ROOT / "contaminacao"
AUTO_REVIEW_BOCA_DIR = AUTO_REVIEW_ROOT / "boca"

# =========================
# GATE ESPACIAL DE CONTAMINANTES NA ABERTURA
# - mantem apenas deteccoes realmente dentro da boca/caçamba alvo
# - mata contaminantes da caçamba vizinha / entorno
# =========================
CONT_OPENING_GATE_ENABLED = True
CONT_OPENING_MIN_IOA = 0.20
CONT_OPENING_MIN_INTERSECTION_PX = 150
CONT_OPENING_MIN_CENTER_INSIDE = True
...
def _sanitize_polygon_points(points, shape_hw):
    if points is None:
        return None
    h, w = shape_hw[:2]
    pts = []
...
def _build_detection_mask_from_geometry(det, shape_hw):
    polygon = None
    bbox = None

    if isinstance(det, dict):
        if det.get("polygon"):
            polygon = _sanitize_polygon_points(det.get("polygon"), shape_hw)
        if det.get("bbox_xyxy") is not None:
            bbox = det.get("bbox_xyxy")
        elif det.get("bbox") is not None:
            bbox = det.get("bbox")
    ...
    if polygon is not None:
        ...
        return mask, bbox_xyxy, "polygon"

    if bbox is not None and isinstance(bbox, (list, tuple, np.ndarray)) and len(bbox) >= 4:
        mask, bbox_xyxy = _build_bbox_mask(shape_hw, bbox[:4])
        return mask, bbox_xyxy, "bbox"

    return None, None, "sem_geometria"


def _filter_contaminant_predictions_to_opening(cont_pred, opening_mask, grupo=""):
    if not CONT_OPENING_GATE_ENABLED:
        return cont_pred
    ...
    if opening_u8 is None or int(np.count_nonzero(opening_u8 > 0)) <= 0:
        out = dict(cont_pred or {})
        out["opening_gate"] = {
            "ativo": False,
            "motivo": "sem_opening_valida",
        }
        return out

    deteccoes = list((cont_pred or {}).get("deteccoes", []) or [])
    if not deteccoes:
        out = dict(cont_pred or {})
        out["opening_gate"] = {
            "ativo": True,
            "motivo": "sem_deteccoes",
            "deteccoes_antes": 0,
            "deteccoes_depois": 0,
            "deteccoes_descartadas": 0,
        }
        return out

    kept = []
    discarded = []
    shape_hw = opening_u8.shape[:2]

    for det in deteccoes:
        det_mask, bbox_xyxy, geom_kind = _build_detection_mask_from_geometry(det, shape_hw)
        det_out = dict(det)
        det_out["geometria_tipo"] = geom_kind

        if det_mask is None:
            det_out["opening_gate_passou"] = False
            det_out["opening_gate_motivo"] = "sem_geometria"
            discarded.append(det_out)
            continue

        det_area = int(np.count_nonzero(det_mask > 0))
        if det_area <= 0:
            det_out["opening_gate_passou"] = False
            det_out["opening_gate_motivo"] = "geometria_vazia"
            discarded.append(det_out)
            continue

        inter = cv2.bitwise_and(det_mask, opening_u8)
        inter_area = int(np.count_nonzero(inter > 0))
        ioa_opening = inter_area / float(max(1, det_area))

        if bbox_xyxy is None:
            bbox_xyxy = _bbox_u8(det_mask)
            if bbox_xyxy is not None:
                bbox_xyxy = bbox_xyxy[:4]

        center_in_opening = False
        if bbox_xyxy is not None:
            x1, y1, x2, y2 = bbox_xyxy
            cx = int((int(x1) + int(x2)) / 2)
            cy = int((int(y1) + int(y2)) / 2)
            center_in_opening = _point_inside_mask(opening_u8, cx, cy)
        else:
            ys, xs = np.where(det_mask > 0)
        ...
        keep = bool(
            (CONT_OPENING_MIN_CENTER_INSIDE and center_in_opening)
            or (inter_area >= CONT_OPENING_MIN_INTERSECTION_PX and ioa_opening >= CONT_OPENING_MIN_IOA)
        )

        det_out["bbox_xyxy"] = list(bbox_xyxy) if bbox_xyxy is not None else det_out.get("bbox_xyxy")
        det_out["opening_intersection_px"] = int(inter_area)
        det_out["opening_ioa"] = round(float(ioa_opening), 4)
        det_out["center_in_opening"] = bool(center_in_opening)
        det_out["opening_gate_passou"] = bool(keep)
        det_out["opening_gate_motivo"] = "ok" if keep else "fora_da_opening"
    ...
    areas_px_by_class = {}
    for det in kept:
        nome = str(det.get("classe", "")).strip().lower()
        if not nome:
            continue
        area_px = float(det.get("area_px", 0.0) or 0.0)
        if area_px <= 0:
            area_px = float(det.get("opening_intersection_px", 0.0) or 0.0)
        areas_px_by_class[nome] = areas_px_by_class.get(nome, 0.0) + max(0.0, area_px)

    total_area = float(sum(areas_px_by_class.values()))
    if total_area > 0:
        areas_ratio = {k: float(v) / total_area for k, v in areas_px_by_class.items()}
    else:
        areas_ratio = {}

    materiais_ordenados = [
        k for k, _ in sorted(areas_ratio.items(), key=lambda x: x[1], reverse=True)
    ]

    if not materiais_ordenados:
        vistos = set()
        for det in kept:
            nome = str(det.get("classe", "")).strip().lower()
            if nome and nome not in vistos:
                vistos.add(nome)
                materiais_ordenados.append(nome)

    kept_enriched = []
    for det in kept:
        nome = str(det.get("classe", "")).strip().lower()
        det2 = dict(det)
        det2["area_ratio"] = round(float(areas_ratio.get(nome, 0.0)), 4)
        kept_enriched.append(det2)

    out = dict(cont_pred or {})
    out["materiais_relevantes"] = materiais_ordenados
    out["materiais_detectados"] = materiais_ordenados
    out["materiais_detectados_brutos"] = list((cont_pred or {}).get("materiais_detectados_brutos", []) or [])
    out["areas_ratio"] = {k: round(float(v), 4) for k, v in areas_ratio.items()}
    out["deteccoes"] = kept_enriched
    out["deteccoes_descartadas_opening"] = discarded
    out["opening_gate"] = {
        "ativo": True,
        "motivo": "ok",
        "deteccoes_antes": len(deteccoes),
        "deteccoes_depois": len(kept_enriched),
        "deteccoes_descartadas": len(discarded),
        "min_ioa": CONT_OPENING_MIN_IOA,
        "min_intersection_px": CONT_OPENING_MIN_INTERSECTION_PX,
    }
    return out
...
def calibrar_fill_operacional(grupo, fill_tecnico):
    ...
    # V3:
    # - manter medio/baixo como estao
    # - empurrar so a faixa alta de plastico, que ainda estava visualmente abaixo
    # - aplicar um segundo ajuste so quando o fill operacional ja estiver alto
    if grupo != "plastico":
        return f
    ...
    base = f
    if f >= 92.0:
        base = min(100.0, f + 1.5)
    elif f >= 88.0:
        base = min(100.0, f + 5.5)
    elif f >= 84.0:
        base = min(100.0, f + 7.0)
    elif f >= 80.0:
        base = min(100.0, f + 7.5)
    elif f >= 74.0:
        base = min(100.0, f + 5.5)
    elif f >= 70.0:
        base = min(100.0, f + 4.0)

    # etapa 2: micro-calibracao so nas altas
    if base >= 94.0:
        return min(100.0, base + 0.5)
    if base >= 90.0:
        return min(100.0, base + 2.5)
    if base >= 86.0:
        return min(100.0, base + 4.5)
    if base >= 82.0:
...
def _ensure_auto_review_dirs():
    AUTO_REVIEW_ROOT.mkdir(parents=True, exist_ok=True)
    AUTO_REVIEW_CONT_DIR.mkdir(parents=True, exist_ok=True)
    AUTO_REVIEW_BOCA_DIR.mkdir(parents=True, exist_ok=True)


def _safe_copy_auto_review(src: Path, dst: Path):
    try:
        if src.exists():
...
def _is_row_contaminacao_candidate(row: dict) -> bool:
    alerta = str(row.get("alerta_contaminacao", "")).strip()
    contaminantes = str(row.get("contaminantes_detectados", "")).strip().lower()
    return alerta == "1" and contaminantes not in ("", "nenhum", "none", "-")


def _is_row_boca_candidate(row: dict) -> bool:
    ...
    motivos_exatos = {
        "sem_boca_modelo_v2",
        "sem_opening_inner_valido",
        "opening_toca_borda",
        "opening_area_fora_ref",
        "suspeito_opening_area",
        "suspeito_borda",
        "expected_overlap_baixo",
        "suspeito_overlap",
    }
    if motivo in motivos_exatos:
        return True

    if "opening" in motivo or "boca" in motivo:
        return True

    return False


def _build_auto_review_cases(row: dict):
    grupo = _slug_auto_review(row.get("grupo", "sem_grupo"))
    cases = []

    if _is_row_contaminacao_candidate(row):
        contaminantes = _slug_auto_review(row.get("contaminantes_detectados", "indefinido"))
        bucket = f"{grupo}__{contaminantes}"
        cases.append(("contaminacao", bucket))

    if _is_row_boca_candidate(row):
        motivo = _slug_auto_review(row.get("motivo_falha", "indefinido"))
        bucket = f"{grupo}__{motivo}"
        cases.append(("boca", bucket))

    return cases


def _append_auto_review_manifest(payload: dict):
    _ensure_auto_review_dirs()
    file_exists = AUTO_REVIEW_MANIFEST.exists()
    try:
        with open(AUTO_REVIEW_MANIFEST, "a", newline="", encoding="utf-8-sig") as f:
...
def auto_collect_review_case(img_path: Path, row: dict):
    cases = _build_auto_review_cases(row)
    if not cases:
        return
    ...
    for category, bucket in cases:
        if category == "contaminacao":
            base_dir = AUTO_REVIEW_CONT_DIR / bucket
        elif category == "boca":
            base_dir = AUTO_REVIEW_BOCA_DIR / bucket
        else:
            continue

        case_dir = base_dir / img_path.stem
        existed_before = case_dir.exists()
        case_dir.mkdir(parents=True, exist_ok=True)

        _safe_copy_auto_review(img_path, case_dir / img_path.name)

        debug_img = DEBUG_DIR / f"{img_path.stem}_debug.jpg"
        _safe_copy_auto_review(debug_img, case_dir / f"{img_path.stem}_debug.jpg")

        opening_mask = OPENING_MASK_DIR / f"{img_path.stem}.png"
        floor_mask = FLOOR_MASK_DIR / f"{img_path.stem}.png"
        wall_mask = WALL_MASK_DIR / f"{img_path.stem}.png"
        _safe_copy_auto_review(opening_mask, case_dir / f"{img_path.stem}_opening.png")
        _safe_copy_auto_review(floor_mask, case_dir / f"{img_path.stem}_floor.png")
        _safe_copy_auto_review(wall_mask, case_dir / f"{img_path.stem}_wall.png")

        try:
            with open(case_dir / f"{img_path.stem}_row.json", "w", encoding="utf-8") as f:
        ...
        if not existed_before:
            payload = {
                ...
                "ts_coleta": datetime.now().isoformat(timespec="seconds"),
            }
            _append_auto_review_manifest(payload)
...
def main():
    ...
    if not new_image_files:
        ...
        run_info = {
            ...
            "auto_review_root": str(AUTO_REVIEW_ROOT),
            "auto_review_manifest": str(AUTO_REVIEW_MANIFEST),
            "auto_review_contaminacao_dir": str(AUTO_REVIEW_CONT_DIR),
            "auto_review_boca_dir": str(AUTO_REVIEW_BOCA_DIR),
    ...
    for img_path, img, opening, floor, wall in stage:
        ...
        if boca_v2_ativo:
            ...
            if not bool(boca_v2_result.get("ok", False)):
                ...
                auto_collect_review_case(img_path, row)
                new_rows.append(row)
        ...
        cont_pred_bruto = segmentador_contaminantes.inferir(str(img_path))
        cont_pred = _filter_contaminant_predictions_to_opening(
            cont_pred_bruto,
            opening_mask=opening,
            grupo=grupo,
        )
        ...
        deteccoes_descartadas_opening = cont_pred.get("deteccoes_descartadas_opening", []) or []
        opening_gate_info = cont_pred.get("opening_gate", {}) or {}
        ...
        deteccoes_contaminantes_json = json.dumps(
            {
                ...
                "deteccoes": deteccoes,
                "deteccoes_descartadas_opening": deteccoes_descartadas_opening,
                "opening_gate": opening_gate_info,
            },
            ensure_ascii=False
        )
        ...
        print(
            ...
            f"materiais={materiais_detectados_raw if materiais_detectados_raw else 'nenhum'} | "
            f"cont_gate_keep={int(opening_gate_info.get('deteccoes_depois', 0) or 0)} | "
            f"cont_gate_drop={int(opening_gate_info.get('deteccoes_descartadas', 0) or 0)}"
        )
    ...
    print("AUTO REVIEW ROOT:", AUTO_REVIEW_ROOT)
    print("AUTO REVIEW MANIFEST:", AUTO_REVIEW_MANIFEST)
    print("AUTO REVIEW CONTAMINACAO:", AUTO_REVIEW_CONT_DIR)
    print("AUTO REVIEW BOCA:", AUTO_REVIEW_BOCA_DIR)
